...
function generatePulseTextureKey(
  x: number,
  y: number,
  base: any,
  pulse: any,
...
function generateShiftTextureKey(x: number, y: number, step: number) {
  return `shift_${x}_${y}_${step}`;
}

export function pregenerateTextures(
  object: {
    appearance: {
      color: {
        base: any;
  ...
  for (let y = 0; y < matrix.length; y++) {
    for (let x = 0; x < matrix[y].length; x++) {
      ...
      if (pulseRate > 0) {
        ...
        sprite.alpha =
          _.get(object, "appearance.opacity", 1) * (1 + phase * amplitude);

        verbose(
          "animationEngine",
          `Pulse at [${x},${y}], phase: ${phase}, scale: ${sprite.scale.x}, alpha: ${sprite.alpha}`
        );
      }
      ...
      if (color?.pulseRate > 0 && color?.pulseColor !== "0") {
        ...
        const ratio = step / (steps - 1);
        const cacheKey = generatePulseTextureKey(
          x,
          y,
          color.base,
          color.pulseColor,
        ...
        if (texture) {
          ...
          verbose(
            "animationEngine",
            `Color pulse at [${x},${y}], phase: ${phase}, color: ${blendColors(
              color.base,
              color.pulseColor,
              ratio
            )}`
      ...
      if (color?.shiftRate > 0) {
        ...
        if (texture) {
          ...
          verbose(
            "animationEngine",
            `Color shift at [${x},${y}], hue: ${adjustedHue}, color: ${hslToHex(
              adjustedHue,
              100,
              50
            )}`
