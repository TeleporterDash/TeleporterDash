...
type NullableHowl = Howl | null;

type VolumeConfig = {
  backgroundMusic: number;
  practiceMusic: number;
  jumpSound: number;
  deathSound: number;
  completionSound: number;
  achievementSound: number;
};
...
class AudioManager {
  backgroundMusic: NullableHowl = null;
  practiceMusic: NullableHowl = null;
  jumpSound: NullableHowl = null;
  deathSound: NullableHowl = null;
  completionSound: NullableHowl = null;
  achievementSound: NullableHowl = null;

  isMuted = false;
  isInitialized = false;
  isMusicPlaying = false;
  restartMusicOnDeath = true;
  restartMusicOnCompletion = true;

  backgroundMusicTime = 0;
  practiceMusicTime = 0;
  lastMusicTime = 0;
   ...
   */
  async initialize(
    levelMusicPath = "../../assets/Sound/Level Soundtracks/practicetd-Dashback"
  ): Promise<void> {
    ...
    try {
      ...
      this.backgroundMusic = new Howl({
        src: [`${levelMusicPath}.mp3`, `${levelMusicPath}.ogg`],
        loop: true,
        volume: this.volumes.backgroundMusic,
        onload: () => debug("audioManager", "Background music loaded"),
        onloaderror: (_id: number, err: unknown) =>
          error("audioManager", "Background music load error:", err),
        onplay: () => {
      ...
      this.practiceMusic = new Howl({
        ...
        onloaderror: (_id: number, err: unknown) =>
          error("audioManager", "Practice music load error:", err),
      });
      ...
      this.jumpSound = new Howl({
        ...
        onloaderror: (_id: number, err: unknown) =>
          error("audioManager", "Jump sound load error:", err),
      });
      ...
      this.deathSound = new Howl({
        ...
        onloaderror: (_id: number, err: unknown) =>
          error("audioManager", "Death sound load error:", err),
      });
      ...
      this.completionSound = new Howl({
        ...
        onloaderror: (_id: number, err: unknown) =>
          error("audioManager", "Completion sound load error:", err),
      });
      ...
      this.achievementSound = new Howl({
        ...
        onloaderror: (_id: number, err: unknown) =>
          error("audioManager", "Achievement sound load error:", err),
      });

      this.isInitialized = true;
      debug(
        "audioManager",
        "Audio manager initialized successfully with Howler.js"
      );
   ...
   */
  async updateBackgroundMusic(levelMusicPath: string): Promise<void> {
    ...
    this.backgroundMusic = new Howl({
      ...
      onloaderror: (_id: number, err: unknown) =>
        error("audioManager", "New background music load error:", err),
      onplay: () => {
        this.isMusicPlaying = true;
      ...
      onpause: () => {
        this.isMusicPlaying = false;
        this.backgroundMusicTime = this.backgroundMusic?.seek() ?? 0;
      },
      onstop: () => {
        this.isMusicPlaying = false;
      },
   ...
   */
  async setup(levelMusicPath?: string): Promise<void> {
  ...
  async waitForLoad(): Promise<void> {
    ...
    const loadPromises = sounds.map((sound) =>
      this.waitForSoundLoad(sound as Howl)
    );
   ...
   */
  waitForSoundLoad(sound: Howl): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const onError = (_id: number, err: unknown) => {
        sound.off("load", onLoad);
        sound.off("loaderror", onError);
        reject(err);
      };
  ...
  updateVolumes(): void {
    ...
    const volumeLevel = slider ? Number(slider.value) / 100 : 0.5;
  ...
  playBackgroundMusic(): void {
    if (!this.isMuted && this.backgroundMusic && !this.isMusicPlaying) {
      verbose(
        "audioManager",
        `Playing background music from ${this.backgroundMusicTime}s`
      );
   ...
   */
  switchTracks(isPracticeMode: boolean): void {
    if (isPracticeMode) {
   ...
   */
  fadeOut(sound: NullableHowl, duration = 1000): Promise<void> {
   ...
   */
  async playCompletionSound(isPracticeMode = false): Promise<void> {
